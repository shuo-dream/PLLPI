    ...
    try:
        ...
        class Args:
            def __init__(self):
                ...
                self.agg_num_layers = agg_num_layers
                # 物理特征文件路径参数
                self.lncrna_physics_path = r'E:\postgraduate\y2025\CWS\my\my_project\PLLPI_PL_B\physics_loss\output\original_physics_feature\original_lncrna_feature.csv'
                self.protein_physics_path = r'E:\postgraduate\y2025\CWS\my\my_project\PLLPI_PL_B\physics_loss\output\original_physics_feature\original_protein_feature.csv'
                # 添加缺失的use_physics_loss属性，默认启用物理损失
        ...
        for epoch in range(args.epochs_num):
            ...
            for batch_idx, batch_data in enumerate(train_batch_data):
                ...
                if args.use_physics_loss and physics_loss_module is not None:
                    # 获取物理特征
                    ...
                    if lncrna_physics is not None and protein_physics is not None:
                        # 计算物理相互作用矩阵
                        physical_matrices = physics_loss_module.compute_physical_matrices(lncrna_physics,
                                                                                          protein_physics)

                        # 将输出转换为概率矩阵形式
                        batch_size_labels = labels.shape[0]
                        # 根据模型输出和标签构造预测交互矩阵
                        predicted_interaction = torch.sigmoid(output).unsqueeze(0).repeat(batch_size_labels, 1)
