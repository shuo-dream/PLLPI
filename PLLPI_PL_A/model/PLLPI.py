...
class DeepFeatureExtractor(nn.Module):
    """
    使用1D卷积神经网络提取深度特征，包含指定层数的卷积层和残差连接
    """
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            if identity.size(1) == out.size(1):  # 通道数匹配
                out = out + identity
            elif residual is not None and residual.size(1) == out.size(1):  # 使用初始残差
                out = out + residual
...
class PLLPI(nn.Module):
    """
    lncRNA-蛋白质相互作用预测模型
    """

    def __init__(self, lncrna_dim=128, protein_dim=128, hidden_dim=64, use_physics_loss=False, dropout_rate=0.2,
                 head_num=8):
        super(PLLPI, self).__init__()
        self.lncrna_dim = lncrna_dim
        self.protein_dim = protein_dim
        self.use_physics_loss = use_physics_loss

        # 深度特征提取器
        self.lncrna_feature_extractor = DeepFeatureExtractor(lncrna_dim, hidden_dim, num_layers=8,
                                                             dropout_rate=dropout_rate)
        self.protein_feature_extractor = DeepFeatureExtractor(protein_dim, hidden_dim, num_layers=8,
                                                              dropout_rate=dropout_rate)

        # 交叉注意力机制
        self.cross_attention = CrossAttention(feature_dim=128, head_num=head_num)
    ...
    def forward(self, batch_data):
        ...
        参数:
        batch_data: 包含lncrna_features和protein_features的批次数据

        返回:
        预测结果
        """
        # 获取特征
        lncrna_features = batch_data['lncrna_features']  # (batch_size, lncrna_feature_dim)
        protein_features = batch_data['protein_features']  # (batch_size, protein_feature_dim)

        # 添加维度以适应卷积层 (batch_size, feature_dim, sequence_length)
        lncrna_features_conv = lncrna_features.unsqueeze(-1)  # (batch_size, lncrna_feature_dim, 1)
        protein_features_conv = protein_features.unsqueeze(-1)  # (batch_size, protein_feature_dim, 1)
        ...
        # 如果需要使用物理损失，返回额外的embedding信息
        if self.use_physics_loss:
            return output, lncrna_weighted, protein_weighted
