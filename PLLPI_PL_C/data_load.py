...
def custom_collate_fn(batch):
    ...
    for key in batch[0].keys():
        if key in ['lncrna_idx', 'protein_idx', 'lncrna_features', 'protein_features', 'label',
                   'lncrna_physics_features', 'protein_physics_features']:
            # 这些是固定大小的张量，可以直接堆叠
...
class generate_postive_negative_labels(Dataset):
    def __init__(self, positive_pairs, negative_pairs, args, aggregated_feature_heterogeneous_graph_data,
        ...
        try:
            # 确定lncRNA物理特征文件路径
            if hasattr(self.args, 'lncrna_physics_path') and self.args.lncrna_physics_path:
                lncrna_physics_path = self.args.lncrna_physics_path
            else:
                # 使用默认路径
                lncrna_physics_path = r'E:\postgraduate\y2025\CWS\my\my_project\PLLPI_PL_C\physics_loss\output\original_physics_feature\original_lncrna_feature.csv'

            # 确定蛋白质物理特征文件路径
            if hasattr(self.args, 'protein_physics_path') and self.args.protein_physics_path:
                protein_physics_path = self.args.protein_physics_path
            else:
                # 使用默认路径
                protein_physics_path = r'E:\postgraduate\y2025\CWS\my\my_project\PLLPI_PL_C\physics_loss\output\original_physics_feature\original_protein_feature.csv'

            # 加载lncRNA物理特征
            if os.path.exists(lncrna_physics_path):
                lncrna_physics_data = pd.read_csv(lncrna_physics_path)
                # 只取数值特征列（排除ID列）
                # 选择数值类型的列，避免object类型导致转换错误
                lncrna_physics_numeric = lncrna_physics_data.select_dtypes(include=[np.number])
                lncrna_physics_values = lncrna_physics_numeric.values
                self.lncrna_physics_features = torch.FloatTensor(lncrna_physics_values.astype(np.float32))
                # 标准化物理特征
                lncrna_physics_mean = self.lncrna_physics_features.mean(dim=0)
                lncrna_physics_std = self.lncrna_physics_features.std(dim=0) + 1e-8
                self.lncrna_physics_features = (self.lncrna_physics_features - lncrna_physics_mean) / lncrna_physics_std
                print(f"成功加载lncRNA物理特征，维度: {self.lncrna_physics_features.shape}")

            # 加载蛋白质物理特征
            if os.path.exists(protein_physics_path):
                protein_physics_data = pd.read_csv(protein_physics_path)
                # 只取数值特征列（排除ID列）
                # 选择数值类型的列，避免object类型导致转换错误
                protein_physics_numeric = protein_physics_data.select_dtypes(include=[np.number])
                protein_physics_values = protein_physics_numeric.values
                self.protein_physics_features = torch.FloatTensor(protein_physics_values.astype(np.float32))
                # 标准化物理特征
                protein_physics_mean = self.protein_physics_features.mean(dim=0)
                protein_physics_std = self.protein_physics_features.std(dim=0) + 1e-8
                self.protein_physics_features = (
                                                            self.protein_physics_features - protein_physics_mean) / protein_physics_std
                print(f"成功加载蛋白质物理特征，维度: {self.protein_physics_features.shape}")
        except Exception as e:
            print(f"加载物理特征时出错: {e}")
            print("将继续使用仅embedding的模式")
        ...
        if self.protein_physics_features is not None:
            print(f"protein物理特征维度: {self.protein_physics_features.shape}")
    ...
    def __getitem__(self, idx):
        ...
        if self.lncrna_physics_features is not None:
            ...
            result['lncrna_physics_features'] = lncrna_physics_tensor
        ...
        if self.protein_physics_features is not None:
            ...
            result['protein_physics_features'] = protein_physics_tensor
            # print(f"protein物理特征已添加到样本中，数据: {protein_physics_tensor}")

        # 打印设备信息
        # print(f"lncrna_tensor use device: {lncrna_tensor.device}")
        # print(f"protein_tensor use device: {protein_tensor.device}")
        # print(f"label_tensor use device: {label_tensor.device}")
